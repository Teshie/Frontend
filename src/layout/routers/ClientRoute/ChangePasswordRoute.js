...
const ChangePasswordRoute = ({ children, user_status, ...rest }) => {
  const redirect = () => {
    try {
      ...
      switch (userType) {
        case 'ClientUser':
          return [<Redirect to="/change_my_password" />, children];
        case 'ClientAdmin':
          return [<Redirect to="/change_my_password" />, children];
        case 'CybermindAdmin':
          return [<Redirect to="/change_my_password" />, children];
...
export default connect(mapStateToProps)(ChangePasswordRoute);
